...
interface DownloadParticle extends Particle {
...
export const downloadEffect: Effect = {
  config: { name: 'download', description: 'ダウンロード + データ流', colors: DEFAULT_COLORS, intensity: 1 },
  create(x: number, y: number, options: EffectOptions = {}): Particle[] {
    const intensity = options.intensity ?? 1;
    const particles: DownloadParticle[] = [];

    // Arrow
    particles.push({
      id: generateId(), type: 'arrow', x, y, progress: 0, maxProgress: 50,
      alpha: 0, size: 20, currentY: y - 30, speed: 2, color: DEFAULT_COLORS[0],
    });

    // Progress bar
    particles.push({
      id: generateId(), type: 'progress', x, y: y + 30, progress: 0, maxProgress: 50,
      alpha: 0, size: 60, currentY: y + 30, speed: 0, color: DEFAULT_COLORS[1],
    });
    ...
    for (let i = 0; i < streamCount; i++) {
      particles.push({
        id: generateId(), type: 'stream', x: x + random(-30, 30), y: y - 60,
        progress: 0, maxProgress: 40, delay: random(0, 30), alpha: 0,
        size: random(2, 4), currentY: y - 60, speed: random(3, 6),
  ...
  update(particle: Particle, deltaTime: number): Particle | null {
    const p = particle as DownloadParticle;
    ...
    if (p.type === 'arrow') {
      p.currentY = p.y - 30 + Math.sin(p.progress * 0.2) * 10;
